...
def process_train_images(paths):
    ...
    for p in paths:
        ...
        for i in range(num_train_augs):
            ...
            aug_fname = p.replace('.jpg', f'_aug_{i:02d}.png')
            
            # aug_00 is the safe direct resize; the rest are random crops
            if i == 0:
                img_out = transform_train_base(img)
            else:
                img_out = transform_train_random(img)
                
            img_out.save(aug_fname);
...
def process_eval_images(paths, split_name):
    ...
    for p in paths:
        ...
        eval_fname = p.replace('.jpg', '_aug_00.png') 
        img_eval = transform_eval(img)
        img_eval.save(eval_fname);
