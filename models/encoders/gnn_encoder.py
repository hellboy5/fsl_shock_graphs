...
class TAGCN_EdgeAugmented(nn.Module):
    ...
    """
    def __init__(self, node_feat_dim, edge_feat_dim, hidden_dim, num_layers, dropout, K_hops=2):
    ...
    def forward(self, data):
        ...
        if USE_TORCH_SCATTER:
            # Accelerated CUDA scatter if torch-scatter is installed
            edge_context = scatter_add(edge_proj, col, dim=0, dim_size=x.size(0))
        else:
            # 100% Native PyTorch C++ equivalent (zero external dependencies)
...
class GraphEncoder(nn.Module):
    def __init__(
        ...
        # 2. Input Projections for Standard Models
        ...
        for _ in range(num_layers):
            if gnn_type == 'GINE':
                ...
                self.layers.append(GINEConv(nn_mlp, edge_dim=hidden_dim))
            elif gnn_type == 'GATv2':
                ...
                # GPS Bug Fix: Removed incompatible 'attn_dropout' argument
